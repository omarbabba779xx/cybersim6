...
def setup_module(module):
    global _dashboard, _logger, _tmpdir
...
class TestDashboard:
    def setup_method(self):
        _logger.clear()
        http_requests.get("http://127.0.0.1:18888/api/replay/live", timeout=3)
    ...
    def test_api_events_invalid_limit_falls_back_safely(self):
        ...
        resp = http_requests.get("http://127.0.0.1:18888/api/events?limit=invalid", timeout=3)
        data = resp.json()
        assert resp.status_code == 200
        assert len(data) == 1
    ...
    def test_api_soc(self):
        ...
        resp = http_requests.get("http://127.0.0.1:18888/api/soc", timeout=3)
        data = resp.json()
    ...
    def test_api_attack_map(self):
        ...
        resp = http_requests.get("http://127.0.0.1:18888/api/attack-map", timeout=3)
        data = resp.json()
    ...
    def test_replay_load_and_step(self):
        ...
        sessions = http_requests.get("http://127.0.0.1:18888/api/replay/sessions", timeout=3).json()
        assert any(item["session_id"] == "replaydemo" for item in sessions)

        state = http_requests.get("http://127.0.0.1:18888/api/replay/load?session=replaydemo", timeout=3).json()
        assert state["mode"] == "replay"
        assert state["session_id"] == "replaydemo"

        stats = http_requests.get("http://127.0.0.1:18888/api/stats", timeout=3).json()
        assert stats["mode"] == "replay"
        assert stats["total_events"] == 2

        reset = http_requests.get("http://127.0.0.1:18888/api/replay/reset", timeout=3).json()
        assert reset["position"] == 0

        stepped = http_requests.get("http://127.0.0.1:18888/api/replay/step?count=1", timeout=3).json()
        assert stepped["position"] == 1

        live = http_requests.get("http://127.0.0.1:18888/api/replay/live", timeout=3).json()
