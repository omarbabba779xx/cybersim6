...
from cybersim.core.config_loader import get_module_config, load_config
...
def _best_effort_stop(service, label: str):
    """Stop a service without masking cleanup failures."""
    try:
        service.stop()
    except Exception as exc:
        print(f"  [!] Cleanup warning for {label}: {exc}")
...
def run_demo(config_path: str = None, with_dashboard: bool = True):
    ...
    print(f"""
{B}{BOLD}{'=' * 54}{RST}
{B}{BOLD}CyberSim6 - AUTOMATED DEMO MODE{RST}
{B}{BOLD}{'=' * 54}{RST}
...
""")
    ...
    try:
        ...
        for index, srv in enumerate(servers, start=1):
            _best_effort_stop(srv, f"server #{index}")
        if dashboard:
            _best_effort_stop(dashboard, "dashboard")
...
def _demo_sqli(config, logger):
    ...
    return {
        "status": "completed",
        "successful": results.get("successful", 0) if results else 0,
...
def _demo_xss(config, logger):
    ...
    return {
        "status": "completed",
        "injected": results.get("injected", 0) if results else 0,
        "findings": len(findings),
    }
...
def _demo_ransomware(config, logger):
    ...
    if not sandbox_dir.exists():
        try:
            ...
            print(f"  [!] Sandbox not available, skipping ransomware encryption: {exc}")
    ...
    print(
        f"  [*] Scan: {scan.get('total_files', 0)} files, "
    ...
    return {
        "status": "completed",
        "encrypted": encrypted,
        "scan": scan.get("is_compromised", False),
    }
...
def _print_final_report(logger, results):
    ...
    print(f"""
{G}{BOLD}{'=' * 54}{RST}
{G}{BOLD}DEMO COMPLETE - FINAL REPORT{RST}
{G}{BOLD}{'=' * 54}{RST}
...
""")
    ...
    for key, color, name in module_display:
        ...
        icon = f"{G}[OK]{RST}" if status == "completed" else f"{D}[--]{RST}"
        details = ""
        if key == "ddos":
            details = f"{result.get('requests', 0)} requests"
        elif key == "sqli":
            details = f"{result.get('successful', 0)} payloads, {result.get('findings', 0)} findings"
        elif key == "bruteforce":
            details = "password found" if result.get("password_found") else "limited demo"
        elif key == "xss":
            details = f"{result.get('injected', 0)} injections, {result.get('findings', 0)} findings"
        elif key == "phishing":
            details = f"{result.get('emails_sent', 0)} emails"
        elif key == "ransomware":
            details = f"{result.get('encrypted', 0)} files encrypted"
